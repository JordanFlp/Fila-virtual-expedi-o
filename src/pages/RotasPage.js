...
function RotasPage() {
  ...
  const handleSolicitar = () => {
    ...
    if (rota.status === "conferida" || rota.status === "em_conferencia" || rota.status === "buffer" || rota.status === "em_fila") {
      mostrarMensagem(
        rota.status === "em_fila"
          ? `Rota ${letter}-${num} já está na fila`
          : `Rota ${letter}-${num} já está em conferência ou concluída`
      );
      return;
    }
  ...
  return (
    <div style={{ padding: "2rem" }}>
      ...
      <div style={{ marginBottom: "16px" }}>
        <label>
          Letra:
          <input type="text" maxLength={1} value={letter} onChange={(e) => setLetter(e.target.value.toUpperCase())} style={{ marginRight: "12px" }} />
        </label>
        <label>
          Número:
          <input type="number" value={num} onChange={(e) => setNum(Number(e.target.value))} style={{ marginRight: "12px" }} />
        </label>
        <button onClick={handleSolicitar}>Solicitar Rota</button>
      </div>

      {mensagem && <div style={{ marginBottom: "24px", padding: "8px", border: "1px solid #ccc", borderRadius: "4px", background: "#f0f0f0" }}>{mensagem}</div>}

      <h2>Bancadas (visualização)</h2>
      <div style={{ display: "flex", flexWrap: "wrap", gap: "16px", marginBottom: "24px" }}>
        {bancadas.map((b) => (
          <div key={b.id} style={{ border: "1px solid #022f40", padding: "12px", borderRadius: "6px", width: "160px", background: b.status === "livre" ? "#5cb85c" : b.status === "em_conferencia" ? "#f0ad4e" : "#d9534f", color: "white" }}>
            <strong>Bancada {b.id}</strong>
            <div>Status: {b.status === "em_conferencia" ? `Em conferência (${b.fila[0]?.letter}-${b.fila[0]?.num})` : b.status}</div>
            <div>{b.fila.length > 0 ? "Fila: " + b.fila.map((r) => `${r.letter}-${r.num}`).join(", ") : "Sem rota"}</div>
          </div>
        ))}
      </div>

      <h2>Fila Global</h2>
      <div style={{ padding: "8px", border: "1px solid #ccc", borderRadius: "4px", minHeight: "50px" }}>
